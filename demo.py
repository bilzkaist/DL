...
def elapse_time_step(distribution):
	...
	if distribution == "gaussian":
		for (idx, coords) in PARTICLE_LOCATION.items():
...
def weight_particles(observation):
	...
	total = 0
	for (idx, location) in PARTICLE_LOCATION.items():
		actual_x = location[0]
		actual_y = location[1]
	...
	for idx, weight in PARTICLE_WEIGHT.items():
...
def get_transition_table_slice(tbl, position):
	start_x = int(CANVAS_WIDTH - position[0])
	end_x   = int(CANVAS_WIDTH + (CANVAS_WIDTH - position[0]))
	start_y = int(CANVAS_HEIGHT - position[1])
	end_y   = int(CANVAS_HEIGHT + (CANVAS_HEIGHT - position[1]))
...
class Application(Canvas):
	...
	def update_particles(self, event=None):
		...
		for (idx, coord) in PARTICLE_LOCATION.items():
